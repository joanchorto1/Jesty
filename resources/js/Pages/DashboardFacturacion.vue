    ...
    <AppLayout>
        <div class="min-h-screen bg-slate-100/80 py-12">
            <div class="max-w-7xl mx-auto px-6">
                <div class="space-y-2">
                    <p class="text-slate-400 text-sm uppercase tracking-widest">Facturación</p>
                    <h1 class="text-3xl sm:text-4xl font-semibold text-slate-900">Control total de presupuestos e ingresos</h1>
                    <p class="text-sm text-slate-500">Detecta tendencias, clientes clave y oportunidades para acelerar los cobros.</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 mt-8">
                    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-5 text-slate-700 shadow-sm xl:col-span-1">
                        <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Presupuestos</p>
                        <p class="text-3xl font-semibold mt-2 text-slate-900">{{ totalBudgets }}</p>
                        <p class="text-sm text-slate-500 mt-3">Importe medio €{{ averageBudget }}</p>
                    </div>
                    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-5 text-slate-700 shadow-sm xl:col-span-1">
                        <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Facturas</p>
                        <p class="text-3xl font-semibold mt-2 text-slate-900">{{ totalInvoices }}</p>
                        <p class="text-sm text-slate-500 mt-3">Ticket medio €{{ averageInvoice }}</p>
                    </div>
                    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-5 text-slate-700 shadow-sm xl:col-span-1">
                        <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Clientes</p>
                        <p class="text-3xl font-semibold mt-2 text-slate-900">{{ totalClients }}</p>
                        <p class="text-sm text-slate-500 mt-3">Top cliente {{ topClientName }}</p>
                    </div>
                    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-5 text-slate-700 shadow-sm xl:col-span-1">
                        <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Cobros pendientes</p>
                        <p class="text-3xl font-semibold mt-2 text-slate-900">€{{ outstandingAmount }}</p>
                        <p class="text-sm text-slate-500 mt-3">{{ outstandingInvoices }} facturas en trámite</p>
                    </div>
                    <div class="rounded-2xl border border-slate-200/80 bg-white/90 p-5 text-slate-700 shadow-sm xl:col-span-1">
                        <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Cobrado</p>
                        <p class="text-3xl font-semibold mt-2 text-slate-900">€{{ totalIncome }}</p>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    <div class="rounded-3xl border border-slate-200/80 bg-white/80 p-6 shadow-sm xl:col-span-2">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-200/70 pb-5">
                            ...
                            <NavLink href="/budgets" class="text-sm font-semibold text-slate-600 hover:text-slate-800">Ver todos</NavLink>
                        </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    <div class="rounded-3xl border border-slate-200/80 bg-white/80 p-6 shadow-sm xl:col-span-2">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-200/70 pb-5">
                            ...
                            <NavLink href="/invoices" class="text-sm font-semibold text-slate-600 hover:text-slate-800">Ver todos</NavLink>
                        </div>
                        <ul class="divide-y divide-slate-200/70 text-sm text-slate-600">
